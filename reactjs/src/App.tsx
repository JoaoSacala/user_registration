...
async function handleDelete(id: string) {
  ...
  return (
   <div className="full min-h-screen bg-gray-900 flex justify-center px-4">
    <main className="my-10 w-full md:max-w-2xl">
      ...
      <form className="flex flex-col my-6" onSubmit={handleSubmit}>
        ...
        <input id="email" type="email"
          ...
          ref={emailRef}
        />
        <input type="submit" value="Cadastrar" className="cursor-pointer w-full bg-green-500 rounded font-medium p-2"/>
      </form>
      ...
      <section className="flex flex-col gap-4">
        {customers.map((customer) => (
          ...
          <p>Status:<span className="font-medium"> {customer.status ? 'Activa' : 'Inactiva'}</span></p>
          <button type="button" className="bg-red-500 w-7 h-7 flex justify-center items-center rounded-lg absolute right-0 -top-2" onClick={() => handleDelete(customer.id)}>
